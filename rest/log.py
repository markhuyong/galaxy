...
from .conf import settings as galaxy_settings
...
class GalaxyFileLogObserver(log.FileLogObserver):
    ...
    def _adapt_eventdict(self, event_dict):
        """Adapt event dict making it suitable for logging with galaxy log
        observer.
        ...
        if event_dict.get('category') == 'scrapy.exceptions.ScrapyDeprecationWarning':
            # ignore Scrapy deprecation warning in galaxy log
            return
...
def setup_logging():
    ...
    if galaxy_settings.LOG_FILE:
        logfile = DailyLogFile.fromFullPath(
            os.path.join(galaxy_settings.LOG_DIR,
                         galaxy_settings.LOG_FILE)
        )
    else:
        logfile = sys.stderr
