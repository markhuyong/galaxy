...
from .conf.spider_settings import get_galaxy_settings, get_project_settings
from .decorators import deprecated
from .log import setup_spider_logging


class GalaxyCrawler(Crawler):
    """Main and only difference from base class -
    galaxyCrawler allows us to call or not call
    start_requests.
...
class GalaxyCrawlerProcess(CrawlerRunner):
    ...
    def crawl(self, spidercls, *args, **kwargs):
        ...
        crawler = GalaxyCrawler(
            spidercls, self.settings, self.galaxy_manager.start_requests)
        self.galaxy_manager.crawler = crawler
        # Connecting signals to handlers that control crawl process
        crawler.signals.connect(self.galaxy_manager.get_item,
                                signals.item_scraped)
        crawler.signals.connect(self.galaxy_manager.collect_dropped,
                                signals.item_dropped)
        crawler.signals.connect(self.galaxy_manager.spider_idle,
                                signals.spider_idle)
        crawler.signals.connect(self.galaxy_manager.handle_spider_error,
                                signals.spider_error)
        crawler.signals.connect(self.galaxy_manager.handle_scheduling,
...
class CrawlManager(object):
    ...
    def crawl(self, *args, **kwargs):
        self.crawler_process = GalaxyCrawlerProcess(
            self.get_project_settings(), self)

        log.msg("========crawl Manager")
        try:
    ...
    def get_project_settings(self):
        ...
        custom_settings = get_galaxy_settings(log_file=log_file)
